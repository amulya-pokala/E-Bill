...
public class CustomerDaoImpl implements CustomerDao {
	...
	public boolean createCustomer(Customer customer) {
		try {
			jdbcTemplate.update(Query.CREATECUSTOMER, customer.getName(), customer.getPassword(), customer.getMailId(),
					customer.getPhoneNo(), customer.getAlias());
	...
	public List<Bill> FilterByMonth(long customerId, String month, String year) {
		return jdbcTemplate.query(Query.FILTERBILLSBYMONTH, new Object[] { customerId, year, month }, new BillMapper());
	...
	public int validateCustomer(String customerName, String password) {
		Integer customerId;
		try {
			customerId = jdbcTemplate.queryForObject(Query.AUTHENTICATIECUSTOMER, new Object[] { customerName, password }, Integer.class);
		} catch (Exception e) {
			customerId = null;
		}
		return customerId;
